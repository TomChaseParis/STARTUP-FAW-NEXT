...
const OrthographeEngine: React.FC<Props> = ({ text }) => {
  const words = text.filter((t): t is Word => typeof t !== "string");
  ...
  return (
    <section className="mt-12">
      ...
      <div className="rounded-xl bg-white p-8 text-[17px] leading-relaxed shadow-lg ring-1 ring-black/5">
        {text.map((part, i) => {
          ...
          const isCorrect =
            answers[index]?.toLowerCase().trim() === part.correct;
      ...
      {showResult && (
        <div className="mt-10 text-center">
          ...
          <p className="font-semibold text-amber-600">{getScoreLevel(score)}</p>
        </div>
      )}
    </section>
  );
