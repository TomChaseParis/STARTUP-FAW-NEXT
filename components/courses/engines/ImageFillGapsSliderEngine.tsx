...
export default function ImageWordInputSlider({
  ...
  const isCorrect = normalize(userAnswer) === normalize(currentItem.answer);
  ...
  const checkAnswers = () => {
    ...
    const finalScore = Math.round((correct / data.items.length) * 100);
  ...
  if (isFinished) {
          ...
          <div className="mx-auto max-w-4xl px-6">
            ...
            <div className="mt-12 text-center">
              <button
                ...
                className="rounded-xl bg-amber-400 px-8 py-3 font-semibold text-black shadow-md transition hover:bg-amber-300"
              >
                Recommencer
              </button>
            </div>
    ...
    return (
      <div className="bg-slate-50 py-24 text-center text-slate-900">
        {isTeacherAnnouncingScore && (
          <div className="mb-10 flex flex-col items-center">
            <div className="relative h-28 w-28 animate-pulse overflow-hidden rounded-full shadow-xl ring-4 ring-amber-400">
              <Image src={teacherImage} alt="" width={112} height={112} />
            </div>

            <div className="mt-6 rounded-2xl bg-amber-100 px-6 py-4 shadow">
        ...
        <div className="mt-10 flex justify-center gap-4">
          <button
            ...
            className="rounded-xl border border-slate-200 bg-white px-8 py-4 text-lg font-semibold shadow-sm transition hover:bg-slate-50"
          >
            Voir mes réponses
          </button>
          ...
          <button
            onClick={() => window.location.reload()}
            className="rounded-xl bg-amber-400 px-8 py-4 text-lg font-semibold text-black shadow-md transition hover:bg-amber-300"
          >
            Recommencer
          </button>
        </div>
      ...
      <div className="flex justify-center">
        {!isValidated ? (
          <button
            ...
            disabled={!hasStarted}
            className="rounded-xl bg-amber-400 px-10 py-4 text-lg font-semibold text-black shadow-md transition hover:bg-amber-300 disabled:opacity-30"
          >
            Vérifier ma réponse
          </button>
        ) : (
          <button
            onClick={next}
            className="rounded-xl bg-amber-500 px-10 py-4 text-lg font-semibold text-black shadow-md transition hover:bg-amber-400"
          >
