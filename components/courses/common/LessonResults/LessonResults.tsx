"use client";

import { useEffect, useRef } from "react";
import { useRouter } from "next/navigation";

import { ActivityResult } from "@/core/activity/models/ActivityResult";
import TeacherFeedback from "@/components/activity/results/TeacherFeedback";
import AnswerHistory from "../ActivityResults/AnswerHistory";
import ResultCard from "../ActivityResults/ResultCard";
import ScoreBadge from "../ActivityResults/ScoreBadge";

type TeacherFeedbackImages = {
  bad: string;
  middle: string;
  good: string;
};

type TeacherFeedbackAudios = {
  bad: string;
  middle: string;
  good: string;
};

type Props = {
  result: ActivityResult;
  onRestart: () => void;

  /**
   * URL vers laquelle l'apprenant revient
   * après avoir terminé la leçon.
   */
  finishHref: string;

  teacherFeedbackImages?: TeacherFeedbackImages;
  teacherFeedbackAudios?: TeacherFeedbackAudios;
};

export default function LessonResults({
  result,
  onRestart,
  finishHref,
  teacherFeedbackImages,
  teacherFeedbackAudios,
}: Props) {
  const router = useRouter();

  const sectionRef = useRef<HTMLElement>(null);

  const score = result.session.score;

  useEffect(() => {
    requestAnimationFrame(() => {
      if (!sectionRef.current) {
        return;
      }

      const y =
        sectionRef.current.getBoundingClientRect().top +
        window.scrollY -
        100;

      window.scrollTo({
        top: y,
        behavior: "smooth",
      });
    });
  }, []);

  const handleFinish = () => {
    router.push(finishHref);
  };

  return (
    <section
      ref={sectionRef}
      className="
        scroll-mt-10
        mx-3
        mt-10
        max-w-5xl
        overflow-hidden
        rounded-3xl
        border
        border-slate-200
        bg-white
        p-4
        shadow-sm
        sm:mx-4
        sm:mt-14
        sm:p-6
        md:mx-auto
        md:mt-20
        md:p-12
      "
    >
      <TeacherFeedback
        score={score}
        variant="lesson"
        teacherFeedbackImages={teacherFeedbackImages}
        teacherFeedbackAudios={teacherFeedbackAudios}
      />

      <header className="text-center">
        <div className="mt-8 sm:mt-10">
          <ScoreBadge
            score={score}
            variant="lesson"
          />
        </div>
      </header>

      <section className="mt-8 grid gap-4 sm:mt-10 sm:gap-6 md:grid-cols-2">
        <ResultCard
          icon="✅"
          label="Réponses correctes"
          value={`${result.session.correctAnswers} / ${result.session.totalQuestions}`}
        />

        <ResultCard
          icon="⏱"
          label="Temps"
          value={`${result.session.duration} s`}
        />

        <ResultCard
          icon="🏆"
          label="Meilleur score"
          value={`${result.bestScore}%`}
        />

        <ResultCard
          icon="🔁"
          label="Tentatives"
          value={`${result.attempts}`}
        />
      </section>

      <AnswerHistory
        history={result.session.history}
      />

      <div className="mt-8 flex flex-col gap-3 sm:mt-14 sm:flex-row sm:justify-center sm:gap-5">
        <button
          type="button"
          onClick={onRestart}
          className="
            w-full
            rounded-xl
            border
            border-slate-300
            px-6
            py-3
            font-semibold
            text-slate-700
            transition
            hover:bg-slate-100
            sm:w-auto
            sm:px-8
          "
        >
          Recommencer
        </button>

        <button
          type="button"
          onClick={handleFinish}
          className="
            w-full
            rounded-xl
            bg-amber-500
            px-6
            py-3
            font-semibold
            text-white
            transition
            hover:bg-amber-600
            sm:w-auto
            sm:px-8
          "
        >
          Terminer la leçon →
        </button>
      </div>
    </section>
  );
}